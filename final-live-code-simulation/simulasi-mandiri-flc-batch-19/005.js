...
function simpleCardsFusion (str) {
  ...
  str = str.split('+'); // Cyber Dragon & Cyber Dragon

  for(let i = 0; i < fusionCards.length; i++) {
    // console.log(fusionCards[i]);
    let card1 = fusionCards[i].card1; // Cyber Dragon
    let card2 = fusionCards[i].card2; // Cyber Dragon

    /**
     * Use this technichique
     * Jadi if (input 1 = x && input2 = y) || input1 =y && input 2=x
     * Jadi if (input 1 = x && input2 = y) || (input1 =y && input 2=x)
     */
    if( (str[0] === card1 && str[1] === card2) || (str[0] === card2 && str[1] === card1) ) {
